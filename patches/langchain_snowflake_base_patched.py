"""
PATCHED Core ChatSnowflake class implementation.

FIXES:
1. Added `disable_streaming` parameter to explicitly disable streaming for tool compatibility

To apply this patch:
    cp patches/langchain_snowflake_base_patched.py \
       .venv/lib/python3.13/site-packages/langchain_snowflake/chat_models/base.py
"""
...
class ChatSnowflake(
    ...
    Key init args — client params:
        ...
        disable_streaming: bool
            Disable streaming for tool calling compatibility (default: False)
            PATCHED: Set to True when using tools to avoid streaming issues.
    ...
    Instantiate:
        .. code-block:: python
            ...
            # PATCHED: Disable streaming for tool compatibility
            llm_with_tools = ChatSnowflake(
                model="claude-3-5-sonnet",
                session=session,
                disable_streaming=True  # Recommended when using tools
            )
    ...
    private_key_passphrase: str | None = Field(default=None)
    ...
    ls_structured_output_format: str | None = Field(default=None)
    ...
    # PATCHED: New streaming control parameter
    disable_streaming: bool = Field(default=False)
    """Disable streaming for tool calling compatibility.

    PATCHED: When set to True, streaming methods will fall back to non-streaming
    invoke() and return the complete response. This is recommended when using
    tools to avoid incomplete responses due to streaming bugs with tool_use deltas.

    Example:
        llm = ChatSnowflake(model="claude-3-5-sonnet", disable_streaming=True)
        llm_with_tools = llm.bind_tools([my_tool])
        # Now .stream() will work reliably even with tools
    """

    @property
    def _ls_structured_output_format(self) -> str | None:
    ...
    def _ls_structured_output_format(self, value: str | None) -> None:
    ...
    def __init__(
        ...
        disable_streaming: bool = False,  # PATCHED: New parameter
        **kwargs: Any,
    ):
        ...
        self.disable_streaming = disable_streaming  # PATCHED: Set new attribute
    ...
    def _identifying_params(self) -> dict[str, Any]:
        ...
        return {
            ...
            "top_p": self.top_p,
            "disable_streaming": self.disable_streaming,  # PATCHED: Include in identifying params
        }
    ...
    def _build_cortex_options_for_sql(self) -> dict[str, Any] | None:
        ...
        try:
            if options:
                ...
                if not execution_result["success"]:
                    SnowflakeErrorHandler.log_and_raise(ValueError(execution_result["error"]), "call Cortex COMPLETE via SQL")
                ...
                if isinstance(formatted_prompt, str):
                    ...
                    prompt_text = " ".join([msg.get("content", "") for msg in formatted_prompt if isinstance(msg, dict)])
                    sql = "SELECT SNOWFLAKE.CORTEX.COMPLETE(?, ?) as response"
                    execution_result = SqlExecutionClient.execute_sync(
                        session=session,
                        sql=sql,
            ...
            if options and response_text.strip().startswith("{"):
                try:
                    ...
                    message = AIMessage(
                        ...
                        usage_metadata=SnowflakeMetadataFactory.create_usage_metadata(input_tokens=input_tokens, output_tokens=output_tokens),
                        response_metadata=SnowflakeMetadataFactory.create_response_metadata(self.model),
                    )
            else:
