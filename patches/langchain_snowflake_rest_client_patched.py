...
class RestApiClient:
    ...
    def _get_base_url(session) -> str:
        ...
        FIX: Check SNOWFLAKE_HOST env var first for SPCS compatibility.
        In SPCS environments, the account name may not directly translate
        to the correct REST API hostname.
        """
        import os
        
        try:
            # FIX: Check SNOWFLAKE_HOST first (required for SPCS)
            snowflake_host = os.environ.get("SNOWFLAKE_HOST")
            if snowflake_host:
                # SPCS sets SNOWFLAKE_HOST - use it directly
