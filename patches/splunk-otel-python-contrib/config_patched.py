...
class Settings:
    ...
    snowflake_cortex_analyst_credits_per_message: float = (
        SNOWFLAKE_CORTEX_ANALYST_CREDITS_PER_MESSAGE_DEFAULT
    )
    snowflake_cortex_search_credits_per_1000_queries: float = (
        SNOWFLAKE_CORTEX_SEARCH_CREDITS_PER_1000_QUERIES_DEFAULT
    )
...
def _parse_float_env(env_var: str, default: float) -> float:
    ...
    try:
        ...
        _logger.warning(
            "Invalid float for %s: %s, using default %s", env_var, value, default
        )
        return default
