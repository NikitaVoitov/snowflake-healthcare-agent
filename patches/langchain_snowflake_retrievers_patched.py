...
class SnowflakeCortexSearchRetriever(BaseRetriever, SnowflakeConnectionMixin):
    ...
    def _parse_rest_api_response(self, data: Dict[str, Any]) -> List[Document]:
        ...
        PATCHED: Now extracts _snowflake_request_id and adds it to each Document's metadata
        for OpenTelemetry tracing visibility.
        """
        documents = []

        # PATCH: Extract Snowflake request ID from response (added by RestApiClient)
        snowflake_request_id = data.get("_snowflake_request_id")
        ...
        for result in results:
            ...
            # PATCH: Add Snowflake request ID to metadata for OTel tracing
            if snowflake_request_id:
                metadata["_snowflake_request_id"] = snowflake_request_id
