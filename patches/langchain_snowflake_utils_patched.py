"""Utility functions for Snowflake chat models - PATCHED VERSION.

PATCHES APPLIED:
1. Added request_id parameter to SnowflakeMetadataFactory.create_response_metadata
   - Enables OTel observability for Snowflake Cortex Inference API calls
   - Maps to gen_ai.response.id (standard OTel semconv)
2. Added guard_tokens parameter for Cortex Guard observability
   - Maps to snowflake.inference.guard_tokens (Snowflake-specific)
"""
...
class SnowflakeMetadataFactory:
    ...
    PATCHED: Added request_id and guard_tokens parameters to create_response_metadata
    for OTel observability of Snowflake Cortex Inference API calls.
    """
    ...
    def create_response_metadata(
        ...
        **kwargs: Any,
        ...
        PATCHED: Added request_id and guard_tokens for OTel observability.

        Args:
            model: Model name used for the response
            finish_reason: Reason the response finished (e.g., 'stop', 'length', 'tool_calls')
            request_id: Snowflake Cortex Inference request ID for observability
                       Maps to gen_ai.response.id (standard OTel semconv)
            guard_tokens: Guard tokens consumed by Cortex Guard (if enabled)
                         Maps to snowflake.inference.guard_tokens (Snowflake-specific)
        ...
        # PATCH: Snowflake Cortex Inference observability metadata
        # These fields are extracted by OTel instrumentation:
        # - snowflake_request_id -> gen_ai.response.id (standard semconv)
        # - snowflake_guard_tokens -> snowflake.inference.guard_tokens (Snowflake-specific)
        if request_id:
            metadata["snowflake_request_id"] = request_id
        if guard_tokens is not None:
            metadata["snowflake_guard_tokens"] = guard_tokens
